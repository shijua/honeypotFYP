...
def test_iter_access_events_skips_bad_lines() -> None:
    events = list(
        iter_access_events(
            [
                "\n",
    ...
    assert len(events) == 1
    assert events[0]["path"] == "/robots.txt"
...
def test_forward_lines_posts_public_portal_events(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    forwarded = forwarder.forward_lines(
        [
            '198.51.100.10 - - [26/Apr/2026:12:00:00 +0000] "GET /assets/app.js.map HTTP/1.1" 404 1 "-" "sqlmap/1.8"\n'
        ],
